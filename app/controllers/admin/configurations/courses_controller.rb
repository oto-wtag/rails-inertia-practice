...
class Admin::Configurations::CoursesController < InertiaController
  before_action :require_admin
  # before_action :set_institution, only: [:destroy]

  def index
    @q = Course.ransack(params[:q])
    @pagy, @courses = pagy(@q.result.order(created_at: :desc), limit: 10)
    ...
    render inertia: "admin/configurations/courses/index", props: {coursesList: courses_list}
  end
